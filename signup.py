...
def handler(event, context):
    ...
    if 'email' in event:
        if not valid_email(event["email"]):
            return_error(400, "Invalid email. The mail must be of type myaccount@example.com")
    else:
        return_error(400, "Email field is empty")

    if 'birthday' in event:
        if not valid_date(event["birthday"]):
            return_error(400, "Birthday is not valid date. The formart must be of type dd/mm/yyyy")
    else:
        return_error(400, "Birthday field is empty")

    if 'role' in event:
        if not valid_role(event["role"]):
            return_error(400, "Invalid role. Only 'teacher' and 'parent' roles are accepted")
    else:
        return_error(400, "Role field is empty")

    if 'phone' in event:
        if not valid_phone(event["phone"]):
            return_error(400, "Invalid phone. This field only accept numbers with a minimum size of 9 ")
    else:
        return_error(400, "Phone field is empty")

    if not 'first_name' in event:
        return_error(400, "First name field is empty")

    if not 'username' in event:
        return_error(400, "Username field is empty")

    if not 'password' in event:
        return_error(400, "Password field is empty")

    if 'last_name' in event:
        if len(event["last_name"]) < 2:
            return_error(400, "Last name field is short")
    else:
        return_error(400, 'Last name is empty')

    if not 'address' in event:
        return_error(400, "Address field is empty")

    if 'postal_code' in event:
        if not valid_postal_code(event["postal_code"]):
            return_error(400, "Invalid code postal or empty. Should be only number with a length between 5 and 7 digits ")
    else:
         return_error(400, "Postal code field is empty")

    if exist_user(event["username"]):
        return_error(400, "Username is already exists")
    ...
    if event["role"] == "parent":
        try:
            ...
            return_error(400, "Invalid format of class or empty. The classroom should be similar to \"1A Infantil\"  ")
        if not exists_classroom(classs=classs, level=level):
            return_error(400, "The classroom doesn't exists on the data base")
    ...
    if event["role"] == "teacher":
        if 'tutor_class' in event:
            try:
                ...
                return_error(400, "Invalid format of class. The classroom should be similar to \"1A Infantil\"  ")

            if not exists_classroom(classs=classs, level=level):
                return_error(400,  "The classroom doesn't exists on the data base ")

            if has_tutor(classs=classs, level=level):
                return_error(400,  "This class already has a tutor")
...
event  = {"username": "magaly", "address": "Getafe", "birthday": "5/12/1992", "email": "editamagalyalvabeltran@gmail.com",
                "first_name": "Randall", "last_name":"Barrientos", "phone": "34654654654", "postal_code": "28028",
                "classroom": "1D Infantil", "role": "parent", "password":"usuario1"}

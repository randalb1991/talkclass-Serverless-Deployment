...
def login(username, password, clientid, db):
    ...
    if response.status_code is not 200:
        print('Response getting auth0 token: ')
        print(str(response.status_code))
        print(str(response.text))
        return response.status_code, json.loads(response.text)['error_description']
    ...
    if response2.status_code is not 200:
        print('Response Delegation: ')
        print(str(response2.status_code))
        print(str(response2.text))
        return response2.status_code, json.loads(response2.text)['error_description']
    ...
    if user is False:
        return 404, "User profile not found on dynamoDB"
    ...
    return 200, json.dumps(body_response)
...
def handler(event, context):
    ...
    if 'role' not in event:
        return return_error(400, "Role not given. should be parent or teacher")

    if 'username' not in event:
        return return_error(400, "Username not given")

    if 'password' not in event:
        return return_error(400, "Password not given")

    if event["role"] == "teacher":
        clientid = os.environ['clientIdTeacher']
        db = os.environ['connectionTeacher']
        status_code, message = login(username=event["username"], password=event["password"], clientid=clientid, db=db)
    if event["role"] == "parent":
        clientid = os.environ['clientIdParent']
        db = os.environ['connectionParent']
        status_code, message = login(username=event["username"], password=event["password"], clientid=clientid, db=db)
    if status_code != 200:
        return return_error(status_code, message)
    else:
        return {
                "statusCode": 200,
                "headers": {
